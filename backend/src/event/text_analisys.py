...
from event.models import Tags  # noqa: E402


class LLMTextAnalysis:
    def __init__(self) -> None:
        self.account = YandexGPTLite(
            "b1gas0roisoqujc22qnc",
            "y0_AgAAAAA85yPjAATuwQAAAAEU6JN6AACgNyQ_6wtCa7zQ-JXuXG27vZvncQ",
        )
    ...
    def extract_date(self, text: str) -> str:
        ...
        return self.account.create_completion(
            task_extract_datetime, temperature="0", system_prompt="Отвечай на русском"
        )
    ...
    def get_time(self, text: str) -> str:
        extract_datetime_query = (
            "Выдели из текста время начало мероприятия. В ответе укажите только время."
        )
        task_extract_datetime = extract_datetime_query + text
        return self.account.create_completion(
            task_extract_datetime, temperature="0", system_prompt="Отвечай на русском"
    ...
    def shorten_text_second(self, text: str) -> str:
        extract_datetime_query = (
            "Сформулируй из этого текста короткий текст для объялвения в афише"
        )
        task_extract_datetime = extract_datetime_query + text
        return self.account.create_completion(
            task_extract_datetime, temperature="0", system_prompt="Отвечай на русском"
    ...
    def is_location(self, text: str) -> str:
        extract_datetime_query = (
            "Это адрес или название места? дай односложный ответ да или нет:"
        )
        task_extract_datetime = extract_datetime_query + text
        return self.account.create_completion(
            task_extract_datetime, temperature="0", system_prompt="Отвечай на русском"
